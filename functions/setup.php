...
if (!function_exists('fk_social_shortcode')) {
    /**
     ...
     * @return string
     */
    function fk_social_shortcode($atts)
    {
        ...
        if (has_social()) {
            ...
            foreach (get_social() as $social) {
                $items .= sprintf(
                    ...
                    sprintf(
                        '<a class="social-link" rel=«nofollow» href="%s" target="_blank"><i class="fa %s" aria-hidden="true" aria-label="%s"></i></a>',
    ...
    add_shortcode('fk-social', 'fk_social_shortcode');
}
